...
class LocalUpdate(object):
    ...
    def train_test(self, dataset, idxs):
        ...
        idxs_train = idxs
        if (self.args.dataset == 'mnist') or (self.args.dataset == 'cifar') or (self.args.dataset=='MRI') or ((self.args.dataset=='chest') or (self.args.dataset=='Glioma') or (self.args.dataset=='Pathology')or (self.args.dataset=='Tumors')):
            idxs_test = idxs
    ...
    def update_weights(self, net, device):
        ...
        for iter in range(self.args.epoch):
            if (iter%5 == 0):
                print('\n', '*' * 2, 'Local Iteration : {}'.format(iter), '*' * 2)
        ...
        for idx, param in enumerate(net.parameters()):
                grad[f"layer_{idx}"]={} 
                grad[f"layer_{idx}"] = param.grad.data

        return grad,w, avg_loss, avg_acc
...
class ServerRetrain(object):
    ...
    def update_weights(self, net, device):
        ...
        # optimizer = torch.optim.ADAM(net.parameters(), lr=self.args.lr, momentum=0)

        epoch_loss = []
        epoch_acc = []
        for iter in range(self.args.retrain_round):
            batch_loss = []
            for i in range(1+int(self.ae_images.size(0)/self.args.batch_size)):
