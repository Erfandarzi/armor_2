...
def noise_injector(x,eps,beta,noise_iter,gamma=1.05):
   
        uniform_noise = torch.rand(x.shape,device=x.device)
        uniform_noise=beta*(2*eps*uniform_noise-eps)
        uniform_noise = uniform_noise / ((1+noise_iter)**gamma)
...
def GC_fast_gradient_method(
    ...
    if noise_inj:
        ...
        for i in range (10):
             x_bank.append(noise_injector(x,eps,beta,noise_iter,gamma))
        for sample in x_bank:
    ...
    # print(x[10].size())
    # x_bank
    # print(torch.norm(x, p='inf', dim=(1,2)))
    d_p =centralized_gradient(x.grad)
    if noise_inj:
        # dp_bank=x_bank.grad
        for i in range(10):
            d_p=x.grad
