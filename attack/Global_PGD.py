...
from attack.GC_fast_gradient_method import GC_fast_gradient_method

from attack.utils import clip_eta


def Global_projected_gradient_descent(
    model_fn,
    x,
    x_update,
    eps,
    ...
    while i < nb_iter:
        ...
        eta = adv_x - x
        # print('before',torch.max(eta))
        eta = clip_eta(eta, norm, eps)
        # print('after',torch.max(eta))
        adv_x = x + eta
        # print('advx',torch.max(adv_x))
