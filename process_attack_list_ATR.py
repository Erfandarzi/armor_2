...
def atr_per_process():
    ...
    if os.path.exists(path_list_processed):
        shutil.rmtree(path_list_processed,onerror=removeReadOnly)
    ...
    for file_list in os.listdir(path_list_processed):
        ...
        if(( file_list.endswith(".out")) and ('attack_list' not in file_list)):
            print("HII")
            ...
            result_file = result_path + "/"+ file_list
            file_data = "Noise\tAATR\n"
            num_items = 100
            if (args.dataset=='Pathology'):
                num_items=100
            if (args.dataset=='Tumors'):
                num_items=179
            if (args.dataset=='MRI'):
                num_items=60
            if (args.dataset=='Glioma'):
                num_items=60
            full_path=path_list_processed +"/"+file_list
            print(file_list)
            ...
            if not os.path.exists(fig_path):
                os.makedirs(fig_path)
            plt.savefig(fig_path +"/"+ model_name +file_list+ "_ATR_noise={:.3f}.pdf".format(args.global_noise_scale))
            plt.close()
