...
def asr_per_process():
    ...
    if (args.dp==True):
                dataset_path = "by_client/dp/" + args.dataset + "/"
    if (args.dp==False):
                dataset_path = "by_client/no_dp/" + args.dataset + "/"
    ...
    #directory ke toosh bara har logfile AATR ro hesab mikone
    ...
    # src_path_AATR_folder=result_path + args.dataset + "_client_num_{}".format(client_num_in_total) 
    # full_path_AATR_folder=path_aatr + args.dataset + "_client_num_{}".format(client_num_in_total)
    for file_list in os.listdir(result_path):
    # for file_acc in os.listdir(path_acc):

        # if file_acc.endswith(".out"):
        #     file_list=file_acc
            # result_file = result_path + args.dataset + "_client_num_{}".format(client_num_in_total) + file_list
            src_path_AATR= result_path+'/'+file_list

            full_path_AATR = path_aatr+'/'+ file_list 
    ...
    for file_acc in os.listdir(path_acc):
            ...
            # for file_accß in os.listdir(path_acc): 
                ...
                print(result_file)
                    ...
                    # plt.tight_layout()
                    # plt.savefig(pic_path + dataset +  "_client_num_{}".format(client_num_in_total) + file_list+ ".pdf")
                    # plt.close()
                    # print(dataset + " Client Num = {} ".format(client_num_in_total) + model_name + " finished")
    print("Saved files in",result_file)
...
if __name__ == '__main__':
    ...
    # args.dataset = "cifar"
    # args.dataset = "mnist"
    # global_noise_scale=args.global_noise_scale
    ...
    for args.model_name in ["noise_adv","no_noise_adv"]:
        for args.dataset in ["Glioma","MRI","Pathology"]:
